...
#define IMG_COLS  174
#define IMG_ROWS 144
#define SQUARE_SIZE 4  // Each square is 4x4 pixels
...
//	  HAL_UART_Transmit_DMA(&huart3, (uint8_t *)msg, strlen(msg));
	...
	// Receive with proper framing
	uint8_t byte;
	int index = 0;
	bool received = false;

	// Simple receive loop with timeout
	uint32_t start = HAL_GetTick();
	while ((HAL_GetTick() - start) < 100) {  // 100ms timeout
	  if (HAL_UART_Receive(&huart5, &byte, 1, 10) == HAL_OK) {
		if (byte == '\n' || index >= sizeof(msg) - 1) {
		  received = true;
		  break;
		}
...
HAL_StatusTypeDef print_msg(char * msg) {
  return HAL_UART_Transmit_DMA(&huart5, (uint8_t *)msg, strlen(msg));
}
